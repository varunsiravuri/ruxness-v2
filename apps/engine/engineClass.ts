...
export class Engine {
  ...
  private snaps: import("mongodb").Collection | any;
    ...
    for (;;) {
      ...
      for (const msg of res[0].messages) {
        ...
        for (const u of batch.price_updates) {
          const { asset, price, decimal } = u as PriceUpdate;
  ...
  private async onUserInit(cmd: any) {
    ...
    if (!this.balances[userId]) {
      this.balances[userId] = { balance: 5000_00, decimal: 2 };
    }
  ...
  private async onTradeOpen(cmd: CreateCmd) {
    ...
    const bal = (this.balances[userId] ??= { balance: 5000_00, decimal: 2 });
    ...
    if (bal.balance < Math.round(marginDollars * 100)) {
      const needCents = Math.round(marginDollars * 100);
      if (bal.balance < needCents) {
        throw new Error("INSUFFICIENT_FUNDS");
      }
    }
    bal.balance -= Math.round(marginDollars * 100);
  ...
  private async onTradeClose(cmd: CloseCmd) {
    ...
    const creditDollars = fromInt4(credit4);
    const creditCents = Math.round(creditDollars * 100);
    (this.balances[userId] ??= { balance: 0, decimal: 4 }).balance +=
      creditCents;
    ...
    // persist closed trade (TODO: add Prisma if you want DB persistence)
    // await prisma.existingTrade.create({ data: { ...mapToDB(order) }});
    ...
    await this.reply(cmd.id, {
      ...
      orderId: order.id,
      pnl4,
      userBalanceCents: this.balances[userId].balance,
    });
  }
  ...
  private async onGetUserBal(cmd: GetUserBalCmd) {
    const userId = String(cmd.userId ?? "");
    if (!userId) throw new Error("userId required");
    const bal = this.balances[userId] ?? { balance: 0, decimal: 2 };
    await this.reply(cmd.id, { balance: bal.balance, decimal: bal.decimal });
  }
    ...
    for (const [userId, orders] of Object.entries(this.openOrders)) {
      ...
      for (const o of [...orders]) {
        ...
        if (pnl4 < -Math.floor(0.9 * lossCapacity4)) {
          const creditDollars = fromInt4(o.margin + pnl4);
          const creditCents = Math.round(creditDollars * 100);
          (this.balances[userId] ??= { balance: 0, decimal: 2 }).balance +=
            creditCents;
          ...
          // await prisma.existingTrade.create({ data: { ...mapToDB(o) }});
